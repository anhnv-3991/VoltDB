...
class NValue {
  ...
  inline void setMdataFromGPU(const char *data) {
	  memcpy(m_data, data, 16);
  }

  inline void setSourceInlinedFromGPU(const bool sourceInlined) {
	  m_sourceInlined = sourceInlined;
  }

  inline void setValueTypeFromGPU(const ValueType val_type) {
	  m_valueType = val_type;
  }
